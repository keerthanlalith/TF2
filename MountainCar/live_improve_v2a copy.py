...
#for i in range(len(FDM_buff_s)):
#    print(FDM_buff_s[i],FDM_buff_a[i],FDM_buff_ns[i],FDM_buff_ns[i]-FDM_buff_s[i])
...
temp = np.zeros((len(test_a),action_dim))
for i in range(len(FDM_buff_a)):
    temp[i][test_a[i]] =1
test_a = temp
    ...
    while((not terminal) and (not human_feedback.ask_for_done()) ):
        ...
        if (feedback_dict.get(h_fb) != 0):  # if feedback is not zero i.e. is valid
            ...
            if(len(AE_buff_s) >= 64): # batch size 64
                #print("Training AE")
                # do you need to run for 5 epochs
                history_AE = AE.fit(x=np.array(AE_buff_s), y=np.array(AE_buff_ns),epochs=100,batch_size=64,shuffle=True, verbose=False)
        ...
        if verbose ==True:
            ...
            print("FDM both next state              ",FDM_ns_both[0],FDM_ns_both[1])
            FDM_ns_l = Kinematic_Model(prev_state,0)
            FDM_ns_r = Kinematic_Model(prev_state,2)
            FDM_ns_both = np.array([FDM_ns_l,FDM_ns_r])
            print("True both next state             ",FDM_ns_both[0],FDM_ns_both[1])
