...
pred_ns_l = FDM.predict([test_s,left])
pred_ns_r = FDM.predict([test_s,right])
for i in range(10):
    print(pred_ns_l[i])
    print(pred_ns_r[i])
...
l = len(test_s)
for i in range(l):
    pred_ns_1.append([pred_ns_l[i],pred_ns_r])
    temp = np.sum(np.abs(pred_ns_1[i]-pred_ns[i]),axis=1)
    cost.append(temp)
    IDM.append(np.argmin(temp, axis=0))


print("predaction")
for i in range(10):
    print(test_a[i],IDM[i])
...
cost = np.abs(pred_ns_1-pred_ns[0])
print(cost)
